...
public abstract class PullToRefreshBase<T extends AbsListView> extends LinearLayout implements OnTouchListener,
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mode == MODE_PULL_UP_TO_REFRESH) {
            Drawable d = getResources().getDrawable(R.drawable.pulltorefresh_up_arrow);
            if (theme.getTheme()==ThemeManager.THEME_DEFAULT) {
               d = Utils.colorDrawable(d, Color.BLACK);
            }            
			headerImage.setImageDrawable(d);
			index = 1;
		} else {
            Drawable d = getResources().getDrawable(R.drawable.pulltorefresh_down_arrow);
            if (theme.getTheme()==ThemeManager.THEME_DEFAULT) {
                d = Utils.colorDrawable(d, Color.BLACK);
            }
            headerImage.setImageDrawable(d);
