 ...
 */
public abstract class QuickActionTreeWidget extends PopupWindow {
    ...
    private OnQuickActionTreeClickListener mOnQuickActionClickListener;
    protected ArrayList<QuickAction> mQuickActions = new ArrayList<QuickAction>();
    private HashMap<QuickAction, ArrayList<QuickAction>> mChildsQuickActions = new HashMap<QuickAction, ArrayList<QuickAction>>();
     ...
     * @author Cyril Mottier
     */
    public static interface OnQuickActionTreeClickListener {
        /**
         ...
         */
        void onQuickActionTreeClicked(QuickActionTreeWidget widget, int parent, int position);
    }
     ...
     */
    public QuickActionTreeWidget(Context context) {
        super(context);
    ...
    /**
     * By default, a {@link QuickActionTreeWidget} is dismissed once the user
     * clicked on a {@link QuickAction}. This behavior can be changed using this
     * method.
     * 
     * @param dismissOnClick True if you want the {@link QuickActionTreeWidget} to
     ...
     * @param listener
     */
    public void setOnQuickActionClickListener(OnQuickActionTreeClickListener listener) {
        mOnQuickActionClickListener = listener;
    }

    public boolean hasChilds(QuickAction action) {
    	return mChildsQuickActions.containsKey(action);
    }
    
    public ArrayList<QuickAction> getChildsQuickACtions(QuickAction action) {
    	if (hasChilds(action)) {
    		return mChildsQuickActions.get(action);
    	}
    	return null;
    }
    
    /**
     * Add a new QuickAction to this {@link QuickActionTreeWidget}. Adding a new
     * {@link QuickAction} while the {@link QuickActionTreeWidget} is currently
     ...
     */
    public void addChildsQuickAction(QuickAction action, ArrayList<QuickAction> list) {
    	addQuickAction(action);
    	mChildsQuickActions.put(action, list);
    }
    ...
    /**
     * Removes all {@link QuickAction} from this {@link QuickActionTreeWidget}.
     */
    public void clearAllQuickActions() {
        if (!mQuickActions.isEmpty()) {
            mQuickActions.clear();
            mChildsQuickActions.clear();
            mIsDirty = true;
        }
    }

    /**
     * Call that method to display the {@link QuickActionTreeWidget} anchored to the
     * given view.
     * 
     * @param anchor The view the {@link QuickActionTreeWidget} will be anchored to.
     */
    ...
    protected OnQuickActionTreeClickListener getOnQuickActionClickListener() {
